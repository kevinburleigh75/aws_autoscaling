...
RSpec.describe 'Protocol#read_records' do
  let(:target_group_uuid)          { SecureRandom.uuid.to_s }
  let(:target_instance_uuid)       { SecureRandom.uuid.to_s }

  let(:target_dead_record_timeout) { 10.seconds }
  let(:dead_records_updated_at)    { Time.now - target_dead_record_timeout - 0.1.seconds }

  let(:action) {
    Protocol::read_records(
      group_uuid:          target_group_uuid,
      instance_uuid:       target_instance_uuid,
      dead_record_timeout: target_dead_record_timeout,
    )
  }
