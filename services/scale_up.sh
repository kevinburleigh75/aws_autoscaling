...
MAX_SIZE="$(aws autoscaling describe-auto-scaling-groups --auto-scaling-group-names $AWS_ASG_NAME --query 'AutoScalingGroups[0].MaxSize' | sed -e 's/"//g')"
echo max_size $MAX_SIZE

aws autoscaling set-desired-capacity --auto-scaling-group-name $AWS_ASG_NAME --desired-capacity $MAX_SIZE --no-honor-cooldown
